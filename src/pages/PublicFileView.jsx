...
const PublicFileView = () => {
  ...
  useEffect(()=>{
    const getFile =async ()=>{
      setIsLoading(true);
      try {
        const res =await axios.get(apiEndpoint.PUBLIC_FILE_VIEW(fileId));
        console.log(apiEndpoint.PUBLIC_FILE_VIEW(fileId));
        setFile(res.data);
        setError(null);
      } catch (error) {
  ...
  return (
    ...
    <header className="p-4 border-b bg-white">
      <div className="container mx-auto flex justify-between items-center">
        ...
        className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg cursor-pointer font-bold">
          <Copy size={18}/>
          Share Link
        </button>
      </div>
      ...
      <div className="w-full max-w-3xl">
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">
            ...
            Shared on {new Date(file.uploadAt).toLocaleDateString()}
          ...
          <div>
            ...
            <div className="text-left text-sm space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-800">{(file.size/1024).toFixed(1)} KB</span>
              
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500" >Shared:</span>
                <span className="text-gray-800">{new Date(file.uploadAt).toLocaleDateString()} </span>
              </div>
